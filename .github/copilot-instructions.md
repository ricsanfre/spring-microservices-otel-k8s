...
- **Kafka consumer deserializer:** Use `org.springframework.kafka.support.serializer.JacksonJsonDeserializer` (Spring Kafka 4, Jackson 3-native) as `value-deserializer` in yaml. The old `JsonDeserializer` is Jackson 2.x-backed and deprecated. Jackson 3 natively supports `java.time.Instant` — no `jackson-datatype-jsr310` dep and no explicit `ConsumerFactory` bean needed. Cross-service type headers: `spring.json.use.type.headers: false` + `spring.json.value.default.type: <local.mirror.Record>`.
