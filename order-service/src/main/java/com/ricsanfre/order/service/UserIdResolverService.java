 ...
 *
 * <p>Per ADR-004 (IAM Portability via user-service Isolation), the order-service must never
...
public class UserIdResolverService {
     ...
     * On a 404 response from {@code user-service}, a {@link ResourceNotFoundException} is thrown.
     *
    ...
    /**
     * Evicts the cached mapping for the given subject (e.g. when a 404 is received on a
     * subsequent call — user was deleted).
     */
    ...
    private UUID resolveInternalIdFallback(String idpSubject, Throwable t) {
        log.warn("Circuit breaker open for user-service — cannot resolve idp_subject={}: {}", idpSubject, t.getMessage());
        throw new ResourceNotFoundException("User", idpSubject);
    }
}
